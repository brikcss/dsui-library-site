...
tim.shell
	...
	.forEach(function(file) {
		...
		// Don't add package.json or package-lock.json. They are either auto-generated or frequently
		// modified by npm.
		if (
			tim.path.basename(filepath) === 'package-lock.json' ||
			tim.path.basename(filepath) === 'package.json'
		) {
			return true;
		}
...
if (files.partiallyUnstaged.length) {
	tim.shell.exec(
		`git diff --ignore-submodules --binary  --no-color --no-ext-diff ${files.partiallyUnstaged.join(
			' '
		)} > ${files.unstagedDiffs} && git checkout -- ${files.partiallyUnstaged.join(' ')}`
	);
}
	...
	promises.push(
		createPromise(
			`node ./node_modules/.bin/prettier ${files.css.join(
				' '
			)} --write --color=always && node ./node_modules/.bin/stylelint ${files.css.join(
				' '
			)} --color always`
		)
	);
	...
	promises.push(
		createPromise(
			`node ./node_modules/.bin/prettier ${files.js.join(
				' '
			)} --write --color=always && node ./node_modules/.bin/eslint ${files.js.join(
				' '
			)} --color always --fix`
		)
	);
...
if (files.json.length) {
	...
	promises.push(
		createPromise(
			`node ./node_modules/.bin/prettier ${files.json.join(' ')} --write --color=always`
		)
	);
} else {
...
function resetStage() {
	...
	if (tim.fs.pathExistsSync(files.unstagedDiffs)) {
		...
		const applyChild = tim.shell.exec(
			`git apply --reject --whitespace=nowarn ${files.unstagedDiffs}`,
			{
				silent: true,
			}
		);
		tim.log.info(
			'     ' + (applyChild.stderr || applyChild.stdout).trim().replace(/\n/g, '\n     ')
		);
	}
}
